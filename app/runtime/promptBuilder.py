...
class PromptBuilder:
    ...
    def buildPrompt(
        ...
        promptText = (
            ...
            "fetchUnread=false and topic derived from the **current User message** (not from session "
            "summary alone), then follow user_topic statuses (needs_channels/needs_keywords). If observation has digest "
            "data_preview.count=0 and data_preview.filteredOutByTime>0, retry digest_telegram_news with "
            "broader sinceHours (72 then 168). For user_topic_telegram_digest: the `topic` argument MUST match "
            "the subject named in the **current User message** (ignore unrelated topics from older turns); "
            "use fetchUnread=false "
            "to configure/check "
            ...
            "topic settings from long-term memory. After one successful user_topic_telegram_digest in this run, "
            "do not call it again with the same topic/args; if the user also asked for a recurring schedule, "
            "next call schedule_recurring_agent_run. For user feedback on liked digest items, follow the "
            ...
            "For save_user_memory_note: use when the active skill is remember_user_note (general facts "
            "the user asked to remember with «запомни/запиши»), not digest or email preferences. "
            "Formulate concise noteText and call the tool once; confirm ok=true before claiming saved.\n"
            ...
            "For schedule_recurring_agent_run: use when the user wants a recurring agent task on an interval "
            "(digest/news/email), not a fixed clock-time Telegram reminder. Put the full user intent into "
            "`message` (what the agent should do each time), e.g. instruct to run user_topic_telegram_digest for "
            "topic «AI». For «каждый день» use intervalSeconds=86400; for «каждый час» use 3600. "
            "If the user names a local hour (e.g. 10 утра), set allowedHourStart and allowedHourEnd to that hour (0–23). "
            "Set intervalSeconds (>=60) and optional "
            "allowedHourStart/allowedHourEnd for local-time windows. Do not use schedule_reminder for "
            "«every hour digest» scenarios. Confirm ok=true from the tool before claiming success.\n"
            "For read_email: default unreadOnly=true; widen sinceHours (up to 168) while keeping "
            "unreadOnly=true if the inbox looks empty. Do not call read_email again with "
            "unreadOnly=false unless the user explicitly asked to include already-read mail.\n"
            f"User message:\n{in_userMessage}\n"
            f"Observations:\n{observationsBlock}\n"
            f"Available tools:\n{in_toolsDescription}\n"
            f"Relevant skills:\n{in_skillsBlock}\n"
    ...
    def buildRoutingPrompt(
        ...
        routingContractText = (
            ...
            "  if BOTH `schedule_recurring_agent_run` and digest skills apply AND the user explicitly asks for a "
            "recurring schedule (каждый день/час, повторяющееся событие, по расписанию, etc.), "
            "`required_first_successful_tool_name` MUST be `schedule_recurring_agent_run` (not user_topic first).\n"
